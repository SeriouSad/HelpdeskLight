...
class IndexView(View):
    ...
    @login_required
    def get(request):
        return render(request, 'index.html', {'groups': [group.name for group in request.user.groups.all()]})
...
class CreateNewTicketView(View):
    ...
    def get(request):
        if request.user.groups.filter(name__in=["Employee", "Operators"]):
            ...
            form = NewTicket()
        return render(request, 'create_ticket.html', {'form': form, 'groups': [group.name for group in request.user.groups.all()]})
        ...
        if request.method == 'POST':
            ...
            return render(request, 'employee/ticket_work.html',
                          {"tickets": tickets, 'form': form, 'link': link,
                           'messages': messages, 'chat': True, "chat_form": chat_form, 'groups': [group.name for group in request.user.groups.all()]})
        else:
            messages = Comment.objects.filter(ticket=ticket)
            form = EditTicketOperator(instance=ticket)
            return render(request, 'employee/ticket_work.html',
                          {"tickets": tickets, 'form': form, 'link': link,
                           'messages': messages, 'chat': True, "chat_form": chat_form, 'groups': [group.name for group in request.user.groups.all()]})
    return render(request, 'employee/ticket_work.html',
                  {"tickets": tickets, 'form': None, 'link': link, 'groups': [group.name for group in request.user.groups.all()]})
